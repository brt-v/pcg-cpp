...
- [x] Target C++17 and use modern best practices
- [x] Convert library and tests to CMake, such that it's easily imported with `FetchContent`
- [x] Make sure it compiles with high warning levels with MSCV, gcc and clang
- [x] Removed many unneeded includes 
- [x] Moved optional funcionality to separate header (`pcg_extras.hpp`) to make the base RNG (`pcg_random.hpp`) a lightweight include
- [x] Convert the tests to CTest
- [x] Fixed some tests on Windows, that fail in the upstream repository
- [x] Add Github actions, make sure tests run on Linux and Windows
- [x] On Widows, use MSVC's built-in 128 bit integer if it's available
- [ ] Add clang format
